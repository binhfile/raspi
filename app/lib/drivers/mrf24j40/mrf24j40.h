...
public:
	MRF24J40(std::string szCmdDevice,
			 int resetGpioNum,
			 int intrGpioNum);
	virtual ~MRF24J40();
	int openDevice();
	...
	int sendData(unsigned short address, const void* data, int length);
	int setChannel(int channel);
	int setPan(unsigned short pan);
	int setAddress16(unsigned short address);

	unsigned short getPan();
	unsigned short getAddress16();
private:
	static void interrupt_run(void* obj, bool* terminate);

	void* m_reset;
	void* m_intr;
	void* m_cmd;
	lib::platform::Thread*		m_interrupt;

	void hardReset();
	void writeShort(unsigned char address, unsigned char data);
	void writeLong(unsigned short address, unsigned char data);
	unsigned char readShort(unsigned char address);
	unsigned char readLong(unsigned short address);

	int writeShortReg(unsigned char address, unsigned char data);
	int writeLongReg(unsigned short address, unsigned char data);
